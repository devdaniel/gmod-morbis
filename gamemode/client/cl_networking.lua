...
local function RecieveOOCChat(UM)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [OOCCHAT]")
	end
...
local function RecieveLocalChat(UM)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [LOCALCHAT]")
	end
...
local function RecieveSpecChat(UM)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [SPECCHAT]")
	end
...
local function RecieveSetPlayer(UM)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [OTHERROLE]")
	end
...
local function RecieveRoundState(UM)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [ROUNDSTATE]")
	end
...
function GM:ClearClientState()
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [RESET]")
	end

	if (not client.SetRole) then  -- Code not loaded
		return
	end
...
function Get_MissionInfo(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [MISSION_INFO]")
	end
...
function Get_MissionInfo_Mini(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [MISSION_INFO_MINI]")
	end
...
function Get_MissionComplete(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [MISSION_COMPLETE]")
	end
...
function ResetMission(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [MISSION_RESET]")
	end
...
function RecieveRoundHistory()
	...
	for k,v in pairs(Round_Log) do
		file.Append("Morbus/logs/"..game.GetMap( ).."/".."round_log_"..tostring(os.date("%m-%d_%H%M"))..".txt","["..v.Time.."] ["..v.Type.."] "..v.Text.."\n")
	end
...
function Get_EvolutionPts(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [GET EVO POINTS]")
	end
...
function Get_Upgrade(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [GET UPGRADES]")
	end
...
function Clear_Upgrades(data)
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [CLEAR UPGRADES]")
	end
	Morbus.Upgrades = {}
end
...
function Get_Transform(data)
	...
	if DEBUG_NET then
		MsgN("MORBUS DEBUG UMSG [TRANSFORM]")
	end
