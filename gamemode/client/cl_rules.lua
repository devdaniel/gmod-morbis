...
function PANEL:Init()
	...
	self.EXIT.Paint = function()
		local amt = 100
		if self.EXIT.Hovered then
			amt = 200
		end
		if self.EXIT.Depressed then
			amt = 250
		end
	...
	for i=1,2 do
		...
		self.Slt[i].Paint = function()
			...
			if (self.Slt[i].Hovered) then
				amt = Color(190,190,10,240)
			end
			if (self.Slt[i].Depressed) then
				amt = Color(240,10,10,240)
			end
			if (self.CurrentTab == i) then
				amt = Color(180,10,10,230)
			end
