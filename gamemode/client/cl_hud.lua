	...
	if trans > 0 then
		...
		cam.Start3D2D( pos, ang, 0.0465)
			...
			if !(ply:Team() == TEAM_SPEC) && !(ply:IsSwarm()) then
				...
				if Mission_End < 0 && Morbus.Mission != MISSION_NONE && Morbus.Mission != MISSION_KILL then
				...
				if Mission_End < 0 then
					Mission_End = ""
				else
					Mission_End = math.abs(Mission_End)
				end
				if Morbus.Mission == 0 then
					Mission_End = ""
				end
				if ply:GetRole() != ROLE_HUMAN then
					Mission_End = ""
				end
