...
function UpdateUpgradesMenu()
	...
	for k,v in pairs(UPGRADES) do
		if !up[v.Tree].ByTier[v.Tier] then
			up[v.Tree].ByTier[v.Tier] = {}
		end
		if !tiers[v.Tree] then
			tiers[v.Tree] = {}
		end
		if !tiers[v.Tree][v.Tier] then
			tiers[v.Tree][v.Tier] = 0
		end
	...
	for tree,info in pairs(up) do
		...
		local tw,th = StringSize("Trebuchet24",UPGRADE_TREES[tree])
		...
		for i=1,mtiers do
			...
			for j=1,ups do
				...
				function obj:Paint()
					...
					if not _TEX[UPGRADES[self.Upgrade].Icon] then
						_TEX[UPGRADES[self.Upgrade].Icon] = surface.GetTextureID(UPGRADES[self.Upgrade].Icon)
					end
					surface.SetTexture(_TEX[UPGRADES[self.Upgrade].Icon])
				...
				function obj:OnCursorEntered()
					if self.DescriptionBox then
						return
					end
					if pUpgradesMenu.Supress then
						return
					end
				...
				function obj:OnCursorExited()
					...
					if not pDescriptionBox then
						return
					end
					if not pDescriptionBox:IsValid() then
						return
					end
				...
				function obj:OnMousePressed()
					if (Morbus.Upgrades[self.Upgrade] == UPGRADES[self.Upgrade].MaxLevel) then
						return
					end
				...
				-- TODO Move locals to top
				local oldy = cury
				cury = cury + UPGRADE_SIZE_MENU + (UPGRADE_MENU_PADDING*0)
				local tw = 24
				if not Morbus.Upgrades then
					Morbus.Upgrades = {}
				end
