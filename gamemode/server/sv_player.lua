...
function GM:NetworkIDValidated( name, steamid ) -- edge case where player authed after initspawn
...
function GM:PlayerSpawn(ply)
	if (GetRoundState() == ROUND_ACTIVE) && (ply.NextSpawnTime && ply.NextSpawnTime > CurTime()) then
		return
	end
...
function GM:Tick() -- three cheers for micro-optimizations
	plys = player.GetAll()
	for i= 1, #plys do
		ply = plys[i]
		tm = ply:Team()
...
function GM:OnPlayerHitGround(ply, in_water, on_floater, speed)
	...
	local damage = math.pow(0.05 * (speed - 400), 1.75) -- Everything over a threshold hurts you, rising exponentially with speed
	if on_floater then
		-- I don't know exactly when on_floater is true, but it's probably when landing on something that is in water.
		damage = damage / 2
	end

	if IsValid(ground) and ground:IsPlayer() then -- if we fell on a dude, that hurts (him) GOOMBASTOMP!
...
function GM:IsSpawnpointSuitable(ply, spwn, force, rigged)
	if not ValidEntity(ply) or not ply:IsGame() then
		return true
	end
	if not rigged and (not ValidEntity(spwn) or not spwn:IsInWorld()) then
		return false
	end
	...
	if not util.IsInWorld(pos + Vector(0,0,40)) then
		return false
	end
