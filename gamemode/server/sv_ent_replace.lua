...
local function ReplaceOnCreated(s, ent)
	if not ent:IsValid() then -- Invalid ents are of no use anyway
		return
	end
...
local function CreateImportedEnt(cls, pos, ang, kv)
	if not cls or not pos or not ang or not kv then
		return false
	end
...
local function ImportSettings(map)
	if not ents.MORBUS.CanImportEntities(map) then
		return
	end
